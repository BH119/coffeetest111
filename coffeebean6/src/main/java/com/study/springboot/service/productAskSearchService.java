...
@Component
public class productAskSearchService {
	@Autowired
	...
	//       1 title
	//       2 writer
	public List<productAskDto> betweenList(
			int type, 
			String keyword, 
			String selectList,
			String page, 
		...
		int totalList = 0;
		if( type == 1 ) {
			totalList = iProductAskDao.titleCount(keyword); //제목 게시글 개수
		}
		else if( type == 2 ) {
		...
		List<productAskDto> list = null;
		if( type == 1 ) {
			list = iProductAskDao.betweenListTitle(startList, endList, keyword);
			...
			list = iProductAskDao.betweenListWrite(startList, endList, keyword);
