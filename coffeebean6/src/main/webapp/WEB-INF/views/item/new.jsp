    ...
    <div class="item">
        ...
        <div class="itemTitle">
            <div class="itemTitleM">
                <h2>NEW</h2>
                <span style="font-size: 18px;">New Items</span>
            </div>
        </div><br><br><br>
       <div class="itemList">
        ...
        <div class="itemPage">
            <ul class="pagination my">
                           <c:choose>
						<c:when test="${page > 1}">
							<a onclick='location.href="/item/new?page=${page-1}"'>이전</a>
						</c:when>
						<c:otherwise>
						<style>#disableLink{pointer-events: none; cursor: default;}</style>
							<a id="disableLink" onclick='location.href="/item/new?page=${page-1}"' >이전</a>
						</c:otherwise>
					</c:choose>

					<c:forEach var="i" begin="${ startPage}" end="${ endPage}">
						<a href="/item/new?page=${i}">${i}</a>
					</c:forEach>

					<c:choose>
						<c:when test="${page < totalPage}">
							<a onclick='location.href="/item/new?page=${page+1}"'>다음</a>
						</c:when>
						<c:otherwise>
						<style> #disableLink{pointer-events: none; cursor: default;}</style>
							<a id="disableLink" onclick='location.href="/item/new?page=${page+1}"'>다음</a>
						</c:otherwise>
					</c:choose>
              </ul>
        </div>
