		...
		<div id="adminMain">
			...
			<div style="display: flex; justify-content: center;">
				<div class="pagination">
					<c:choose>
						<c:when test="${page > 1}">
							<a onclick='location.href="item_view?page=${page-1}&selectList=${ selectList}&keyword=${keyword}"'>이전</a>
						</c:when>
						<c:otherwise>
						<style>#disableLink{pointer-events: none; cursor: default;}</style>
							<a id="disableLink" onclick='location.href="item_view?page=${page-1}&selectList=${ selectList}&keyword=${keyword}"' >이전</a>
						</c:otherwise>
					</c:choose>

					<c:forEach var="i" begin="${ startPage}" end="${ endPage}">
						<a href="item_view?page=${i}&selectList=${ selectList}&keyword=${keyword}">${i}</a>
					</c:forEach>

					<c:choose>
						<c:when test="${page < totalPage}">
							<a onclick='location.href="item_view?page=${page+1}&selectList=${selectList}&keyword=${keyword}"'>다음</a>
						</c:when>
						<c:otherwise>
						<style> #disableLink{pointer-events: none; cursor: default;}</style>
							<a id="disableLink" onclick='location.href="item_view?page=${page+1}&selectList=${selectList}&keyword=${keyword}"'>다음</a>
						</c:otherwise>
					</c:choose>
				</div>
			</div>
